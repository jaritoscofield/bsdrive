...
<div class="p-6 max-w-4xl mx-auto">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            ...
            <p class="text-neutral-600">{{ $user->name }} - Edite as informações da pasta</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-neutral-200">
        <div class="p-6">
            <form action="{{ route('users.folders.update', [$user, $userFolder]) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" value="{{ $userFolder->google_drive_folder_id }}" class="w-full px-3 py-2 border border-neutral-300 rounded-lg bg-neutral-50 text-neutral-500" readonly>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="folder_name" id="folder_name" value="{{ old('folder_name', $userFolder->folder_name) }}" class="w-full px-3 py-2 border border-neutral-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-neutral-500 focus:border-neutral-500" placeholder="Nome da pasta para referência" required>
                    @error('folder_name')
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3" class="w-full px-3 py-2 border border-neutral-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-neutral-500 focus:border-neutral-500" placeholder="Descreva a pasta...">{{ old('description', $userFolder->description) }}</textarea>
                    @error('description')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="permission_level" id="permission_level" class="w-full px-3 py-2 border border-neutral-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-neutral-500 focus:border-neutral-500" required>
                        ...
                        <option value="read" {{ old('permission_level', $userFolder->permission_level) === 'read' ? 'selected' : '' }}>Leitura - Apenas visualizar arquivos</option>
                        <option value="write" {{ old('permission_level', $userFolder->permission_level) === 'write' ? 'selected' : '' }}>Escrita - Visualizar e editar arquivos</option>
                        <option value="admin" {{ old('permission_level', $userFolder->permission_level) === 'admin' ? 'selected' : '' }}>Administrador - Controle total da pasta</option>
                    </select>
                ...
                <div>
                    <label class="flex items-center">
                        <input type="checkbox" name="active" value="1" {{ old('active', $userFolder->active) ? 'checked' : '' }} class="h-4 w-4 text-neutral-600 focus:ring-neutral-500 border-neutral-300 rounded">
