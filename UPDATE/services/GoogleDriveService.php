...
class GoogleDriveService
{
    ...
    public function __construct()
    {
        ...
        $this->sharedDriveId = config('services.google.shared_drive_id');
    ...
    public function uploadFile($filePath, $fileName, $parentId = null, $mimeType = null)
    {
        ...
        // Verificar se temos Shared Drive configurado
        if (empty($this->sharedDriveId)) {
            \Log::emergency('⚠️  AVISO: Shared Drive ID não configurado - tentando upload sem Shared Drive');
            \Log::emergency('💡 Isso pode falhar por quota, mas vamos tentar...');
        }
        ...
        try {
            ...
            // Configurar parâmetros baseado na disponibilidade do Shared Drive
            $optParams = [
                'data' => $content,
                'mimeType' => $mimeType,
            ...
            // Só adiciona parâmetros de Shared Drive se tiver configurado
            if (!empty($this->sharedDriveId)) {
                $optParams['supportsAllDrives'] = true;
                \Log::emergency('📤 Upload para Shared Drive: ' . $this->sharedDriveId);
            } else {
                \Log::emergency('📤 Upload SEM Shared Drive (modo compatibilidade)');
            }
            
            \Log::emergency('📤 Iniciando upload', [
                'fileName' => $fileName,
                'parentId' => $parentId,
                'hasSharedDrive' => !empty($this->sharedDriveId),
                'sharedDriveId' => $this->sharedDriveId ?: 'N/A',
